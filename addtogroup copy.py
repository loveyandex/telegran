...
def my_event_handler(event):
    ...
    if isinstance(message.action, MessageActionChatDeleteUser) or isinstance(message.action, MessageActionChatAddUser):
        result = client(DeleteMessagesRequest(
            target_group_entity, [message.id]))
        print(result)
...
for user in b[270:]:
    ...
    randrange = random.randrange(0, 2)
    k = k + 1
    try:
        # Note that ``user_to_add`` is NOT the name of the parameter.
        # It's the user you want to add (``user_id=user_to_add``).
        ...
        for message in client.iter_messages(chats):
            if isinstance(message.action, MessageActionChatDeleteUser) or isinstance(message.action, MessageActionChatAddUser):
                result = client(DeleteMessagesRequest(target_group_entity, [message.id]))

        print(f"{k}th {user.first_name} , {user.id}Waiting for {randrange} Seconds...")
